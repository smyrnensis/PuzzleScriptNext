...
var Compiler3DOverlay = (function(root) {
'use strict';
...
function generateExtraMembers(state) {
    ...
    for (const [key, obj] of Object.entries(state.objects)) {
        ...
        if (isThreeDimensionsEnabled(state) && !obj.sprite3matrix) {
            obj.sprite3matrix = createDefaultSprite3Matrix();
            obj.sprite3offset = obj.sprite3offset || { row: 0, col: 0, slice: 0 };
        }
    }

    function createDefaultSprite3Matrix() {
        const width = state.sprite_size;
        const depth = state.cell_height || state.sprite_size;
        const height = state.sprite_depth || state.sprite_size;
...
function getCompiler3DApi() {
    if (typeof require === "function") {
        try {
            return require("./compiler_3d.js");
        } catch (err) {
            // Browser builds provide the API on the global object instead.
        }
    }
    if (typeof Compiler3D !== "undefined")
        return Compiler3D;
    if (typeof window !== "undefined" && window.Compiler3D)
        return window.Compiler3D;
    throw new Error("Compiler3D helper is required before compiler.js.");
}

function compiler3DDeps() {
    return {
        BitVec,
        STRIDE_OBJ,
        levelEntriesToArray,
        isThreeDimensionsEnabled,
        logError,
        logErrorNoLine,
        logWarning,
        fixUpGosubs,
        getRuleLoweringApi,
        getRuleFinalizationApi,
        getStrideObj: function(state) {
...
function lowerThreeDimensionLevels(state) {
    return getCompiler3DApi().lowerThreeDimensionLevels(state, compiler3DDeps());
}

function levelFromThreeDimensionParsedSource(state, parsedLevel) {
    return getCompiler3DApi().levelFromThreeDimensionParsedSource(state, parsedLevel, compiler3DDeps());
}
...
function rulesToMask3D(state, rules) {
    return getCompiler3DApi().rulesToMask3D(state, rules, compiler3DDeps());
}
...
function finalizeRulesFor3D(state) {
    return getCompiler3DApi().finalizeRulesFor3D(state, compiler3DDeps());
}
...
function startCompiledState(state, command, randomseed) {
    ...
    if (state && Array.isArray(state.hostCapabilities) && state.hostCapabilities.length > 0) {
        const capabilities = getPuzzleHostCapabilities();
        if (!capabilities || typeof capabilities.startCompiledState !== "function") {
            reportHostPreparationError(new Error("Browser playback host is missing startCompiledState()."));
            consoleCacheDump();
            return null;
        }
        const result = capabilities.startCompiledState(state, command, randomseed);
        consoleCacheDump();
        return result;
    }
...
const Compiler3DOverlay = {
    ...
    startCompiledState,
    restoreProcessInput2DBridge,
    __setCompilerCarriersForTest(state) {
        STRIDE_OBJ = state.STRIDE_OBJ;
        STRIDE_MOV = state.STRIDE_MOV;
        MOV_BITS = state.MOV_BITS;
        MOV_MASK = state.MOV_MASK;
    },
    __compilerInternalsForTest: {
        expandRulesWithPrefixes,
        expandRulesWithTags,
        expandRulesWithMultiDirectionObjects,
        expandRulesWithMultipleDirections,
        convertObjectsAndDirections,
        finalizeRulesFor2D,
        finalizeRulesFor3D
    }
};
...
var loadFile = Compiler3DOverlay.loadFile;
var getTag = Compiler3DOverlay.getTag;
var getMapping = Compiler3DOverlay.getMapping;
var applyTransforms = Compiler3DOverlay.applyTransforms;
var wordAlreadyDeclared = Compiler3DOverlay.wordAlreadyDeclared;
var isDeclaredAs = Compiler3DOverlay.isDeclaredAs;
...
var lowerThreeDimensionLevels = Compiler3DOverlay.lowerThreeDimensionLevels;
var levelFromThreeDimensionParsedSource = Compiler3DOverlay.levelFromThreeDimensionParsedSource;
var rulesToMask3D = Compiler3DOverlay.rulesToMask3D;
var hasThreeDimensionLevels = Compiler3DOverlay.hasThreeDimensionLevels;
var inferHostCapabilities = Compiler3DOverlay.inferHostCapabilities;
var attachHostCapabilities = Compiler3DOverlay.attachHostCapabilities;
var prepareCompiledStateForHost = Compiler3DOverlay.prepareCompiledStateForHost;
var startCompiledStateAfterHostPreparation = Compiler3DOverlay.startCompiledStateAfterHostPreparation;
var compileAndStart = Compiler3DOverlay.compileAndStart;
var compileToState = Compiler3DOverlay.compileToState;
var startCompiledState = Compiler3DOverlay.startCompiledState;
var restoreProcessInput2DBridge = Compiler3DOverlay.restoreProcessInput2DBridge;
...
if (typeof module !== 'undefined' && module.exports)
    module.exports = Compiler3DOverlay;
