...
////////////////////////////////////////////////////////////////////////////////
    ...
    function drawObjects(render) {
        ...
        if (typeof is3DLevelEditorActive === "function" && is3DLevelEditorActive()) {
            draw3DLevelEditorObjects(render);
            return;
        }
    ...
    function draw3DLevelEditorObjects(render) {
        const posindexes = [];
        const seen = {};
        for (const group of state.collisionLayerGroups) {
            for (const posindex of render.getPosIndexes(group)) {
        ...
        for (const posindex of posindexes) {
            const posmask = curlevel.getCellInto(posindex, _o12);
            const entries = [];
            for (const group of state.collisionLayerGroups) {
                for (let k = group.firstObjectNo; k < group.firstObjectNo + group.numObjects; ++k) {
                    ...
                    if (showLayers && obj.layer != showLayerNo)
                        continue;
                    if (!obj.sprite3matrix)
                        throw new Error("3D level editor requires sprite3matrix data for object \"" + state.idDict[k] + "\".");
                    entries.push({
                        matrix: obj.sprite3matrix,
                        colors: obj.colors.map(c => colorToHex(state.metadata.color_palette, c))
                    });
                }
            }
            if (entries.length === 0)
            ...
            const sprite = sprite3MatrixTopDownLayeredSprite(entries);
            if (sprite.dat.length === 0)
                continue;
            const image = createSprite("levelEditor3DCell" + posindex, sprite.dat, sprite.colors, state.sprite_size, state.cell_height);
            const drawpos = render.getDrawPos(posindex, {
                spriteoffset: { x: 0, y: 0 },
                spritematrix: sprite.dat
            });
            const spriteSize = {
                w: sprite.dat.reduce((acc, row) => Math.max(acc, row.length), 0) * pixelSize,
                h: sprite.dat.length * pixelSize,
            };
            ctx.drawImage(
                image, 0, 0, spriteSize.w, spriteSize.h,
                Math.floor(drawpos.x), Math.floor(drawpos.y), spriteSize.w, spriteSize.h);
        }
    }
...
function canvasResize(level) {
    ...
    if (textMode) {
        ...
        if (typeof isCurrentLevel3D !== "function" || !isCurrentLevel3D())
