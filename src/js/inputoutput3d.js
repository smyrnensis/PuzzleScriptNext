...
function getInputOutputLevels() {
	return state && state.levels || [];
}

function isGameCanvasTarget(target) {
	return target === canvas || (target && target.id === "gameCanvas3D");
}

function blurBrowserGameInputFocus(target) {
	if (!isGameCanvasTarget(target) && !(target && target.className==="tapFocusIndicator"))
		return;
	...
	if (typeof editor !== "undefined"
		...
		&& typeof editor.display.input.blur === "function")
		editor.display.input.blur();
}

function getInputOutputHostCapabilities() {
...
function processBrowserPlayInput(inputdir, dontDoWin, dontModify, bak, coordIndex) {
	const capabilities = getInputOutputHostCapabilities();
	if (capabilities
		&& typeof capabilities.hasActiveBrowserSession === "function"
		&& capabilities.hasActiveBrowserSession()) {
		if (typeof capabilities.processBrowserInput !== "function")
			throw new Error("Active browser playback host is missing processBrowserInput().");
		return capabilities.processBrowserInput(inputdir, dontDoWin, dontModify, bak, coordIndex);
	}
	return processInput(inputdir, dontDoWin, dontModify, bak, coordIndex);
}

function processBrowserSessionCommand(command) {
	const capabilities = getInputOutputHostCapabilities();
	if (capabilities
		&& typeof capabilities.hasActiveBrowserSession === "function"
		&& capabilities.hasActiveBrowserSession()) {
		if (typeof capabilities.handleSessionCommand !== "function")
			throw new Error("Active browser playback host is missing handleSessionCommand().");
		return capabilities.handleSessionCommand(command);
	}
	return false;
}
...
function levelEditorClick(event,click) {
	if (is3DLevelEditorActive())
		return levelEditor3DClick(event, click);
...
function levelEditorRightClick(event,click) {
	if (is3DLevelEditorActive())
		return levelEditor3DRightClick(event, click);
...
function mouseAction(event,click,id) {
	...
	if (textMode) {
		...
		} else if (messageselected===false && (getInputOutputLevels()[curLevelNo].message || messagetext != "")) {
...
function mouseInput(id, coordIndex) {
	if (id >= 0) {
		...
		try {
			...
			var inputdir = 5;
			if (processBrowserPlayInput(inputdir,false,false,bak)) {
				redraw();
			}
		} catch(e) {
			console.log(e);
		...
		if (processBrowserPlayInput(inputdir, false, false, bak, coordIndex)) {
			redraw();
		}
	}
}
...
function onMouseDown(event, wasFiredByTouch = false) {
	...
	if (lmb ) {
		...
		blurBrowserGameInputFocus(event.target);
		keybuffer=[];
		if (isGameCanvasTarget(event.target) || event.target.className==="tapFocusIndicator") {
			setMouseCoord(event);
			dragging=true;
			rightdragging=false;
		...
		if (isGameCanvasTarget(event.target)) {
			blurBrowserGameInputFocus(event.target);
			setMouseCoord(event);
			dragging=false;
			rightdragging=true;
...
function onKeyDown(event) {
    ...
    if(isGameCanvasTarget(lastDownTarget) || (window.Mobile && (lastDownTarget === window.Mobile.focusIndicator) ) ){
...
function checkKey(e,justPressed) {
    ...
    switch(e.keyCode) {
        ...
        case 90://z
        {
            ...
            if (!textMode) {
                pushInput("undo");
                if (!processBrowserSessionCommand("undo"))
	                DoUndo(false,true);
                canvasResize(); // calls redraw
            	return prevent(e);
            }
        ...
        case 82://r
        {
        	if (!textMode) {
        		if (justPressed) {
	        		pushInput("restart");
	        		if (!processBrowserSessionCommand("restart"))
		        		DoRestart();
	                canvasResize(); // calls redraw
            		return prevent(e);
            	}
        ...
        case 69: {//e
        	if (!solving && canOpenEditor) {
        		if (justPressed) {
					var finishLevelEditorToggle = function() {
						levelEditorOpened=!levelEditorOpened;
						if (levelEditorOpened && typeof prepareLevelEditorForCurrentLevel === "function" && prepareLevelEditorForCurrentLevel())
							return;
					...
					var levelStartResult = null;
        			if (titleScreen){
        				if (state.title==="EMPTY GAME"){
							levelStartResult = compile(["loadLevelEditor",0]);
						} else if (!isCurrentLevelEditor3D()) {
							levelStartResult = nextLevel();
        				}
        			}
					if (levelStartResult && typeof levelStartResult.then === "function")
						levelStartResult.then(finishLevelEditorToggle);
					else
						finishLevelEditorToggle();
        		}
        		return prevent(e);
        	}
            break;
    ...
    if (textMode) {
    	...
    	if (getInputOutputLevels().length===0) {
    		//do nothing
    	} else if (titleScreen) {
	    ...
	    if (!againing && inputdir>=0) {
			if (suppressInput) {
                ...
                pushInput(inputdir);
                if (processBrowserPlayInput(inputdir)) {
                    redraw();
                }
	        }
	       	return prevent(e);
...
function update() {
    ...
    if (quittingMessageScreen) {
        ...
        if (timer/1000>0.15) {
            ...
            if (getInputOutputLevels()[curLevelNo].message) {
				nextLevel();
            } else {
            	messagetext="";
            	textMode=false;
